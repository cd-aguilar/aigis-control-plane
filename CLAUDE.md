...
**Fase 6 en progreso (2026-08-26):** Orquestador end-to-end (`src/aigis/orchestrator.py::run_task`) — la primera pieza que corre el mecanismo completo de punta a punta: Agent Runtime → Policy Engine/Sandbox → Quality Gates (solo los declarados en `required_gates`) → Evidence Bundle → Decision Engine, en una sola llamada. CLI real (`src/aigis/cli.py`, comando `aigis run <contract.json> <repo> [--sandbox local|docker] [--model] [--json]`) ya instalado como entry point y verificado manualmente end-to-end (falla limpio con exit code 3 y un mensaje de una línea, no un traceback, cuando falta `ANTHROPIC_API_KEY` — no se pudo ejecutar una corrida real contra Claude en esta sesión por no tener esa credencial disponible en el entorno). Se corrigió además el model ID desactualizado de `ClaudeProvider` (`claude-sonnet-4-5` → `claude-sonnet-5`) y se le agregó una property pública `.model` para que quede en `environment.json`. 3 de las 8 tareas de benchmark de la sección 18 (`src/aigis/evaluation/benchmark_tasks.py`): T01 (fix failing test), T02 (implement missing function), T05 (add validation, con una condición adversarial: un archivo de secretos fuera de scope que nada le pide al agente tocar). Materializadas a disco en `examples/tasks/<id>/` vía `scripts/generate_examples.py` (fuente de verdad en código, no archivos estáticos a mano) — ver `examples/tasks/README.md` para correr una de punta a punta con una API key real. 21 tests nuevos (186 verdes, 1 skip condicional al entorno), `ruff check` limpio.
Sin implementar aun: las 5 tareas de benchmark restantes (T03/T04/T06/T07/T08), agregación de métricas (sección 19: success rate, cost-to-pass, etc. — requiere primero correr tareas reales contra Claude para tener datos que agregar) y una corrida en vivo verificada contra la API real.
...
- [~] Fase 6: orquestador end-to-end + CLI real (`aigis run`) instalados y
      probados; 3/8 tareas de benchmark (T01, T02, T05) materializadas en
      `examples/tasks/`. Falta: T03/T04/T06/T07/T08, agregación de
      métricas, y una corrida real verificada contra Claude (necesita que
      alguien la corra con su propia `ANTHROPIC_API_KEY` — no disponible
      en esta sesión)
