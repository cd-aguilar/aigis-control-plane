...
- **Fase 6 en progreso (2026-08-26):** Orquestador end-to-end (`src/aigis/orchestrator.py::run_task`) — corre el mecanismo completo de la sección 1 en una sola llamada: Agent Runtime → Policy Engine/Sandbox → Quality Gates (solo los declarados en `required_gates`) → Evidence Bundle → Decision Engine. CLI real (`src/aigis/cli.py`, `aigis run <contract.json> <repo>`) instalado como entry point de `pyproject.toml` y verificado manualmente: falla limpio con exit code 3 y un mensaje de una línea (no un traceback) cuando falta `ANTHROPIC_API_KEY`. Se corrigió el model ID desactualizado de `ClaudeProvider` (`claude-sonnet-4-5` → `claude-sonnet-5`). 3 de las 8 tareas de la sección 18 (T01, T02, T05 — esta última con una condición adversarial: un archivo de secretos fuera de `allowed_paths`/dentro de `forbidden_paths` que nada le pide al agente tocar), materializadas en `examples/tasks/` vía `scripts/generate_examples.py`.
- Estado de tests verificado el 2026-08-26: **186 tests pasando, 1 skip condicional al entorno**, `ruff check` limpio.

**Pendiente:**
- Fase 6: T03/T04/T06/T07/T08 (5 tareas de benchmark restantes), agregación de métricas de la sección 19 (requiere primero correr tareas reales contra Claude para tener datos que agregar), y una corrida end-to-end verificada contra la API real (necesita `ANTHROPIC_API_KEY`, no disponible en el entorno donde se construyó esta fase).
